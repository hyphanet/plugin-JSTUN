...
public class JSTUN
        implements FredPlugin, FredPluginIPDetector, FredPluginThreadless, FredPluginHTTP,
    ...
    static String[] publicSTUNServers = new String[] {
        "stun.voipbuster.com", "stun.ekiga.net", "stun.ideasip.com", "stun.iptel.org",
        "stun.rixtelecom.se", "stun.schlund.de", "stun.softjoys.com", "stun.voiparound.com",
        "stun.voipstunt.com", "stun.voxgratia.org", "stun.xten.com", "stunserver.org",
    };
    ...
    DetectedIP[] runTest(InetAddress iaddress) {
        ...
        while ( !v.isEmpty()) {
            ...
            try {
                ...
                Logger.normal(this,
                              "Successful STUN discovery from " + stunServer + "!:" + reportedData
                              + " likely detections: " + countLikely + " unlikely detections "
                              + countUnlikely + " remaining " + v.size());
                System.err.println("Successful STUN discovery from " + stunServer + "!:"
                                   + reportedData + " likely detections: " + countLikely
                                   + " unlikely detections " + countUnlikely + " remaining "
                                   + v.size());
                ...
                if (ip == null) {
                    Logger.normal(this,
                                  "Failed to parse reported data, skipping server " + stunServer);
                ...
                if ((ip.natType == DetectedIP.NO_UDP) || (ip.natType == DetectedIP.NOT_SUPPORTED)
                        || (ip.natType == DetectedIP.SYMMETRIC_NAT)
                        || (ip.natType == DetectedIP.SYMMETRIC_UDP_FIREWALL)) {
                    countUnlikely++;    // unlikely outcomes
                } else {
                    countLikely++;
                }

                if ((countUnlikely >= 3) || (countLikely >= 2) || v.isEmpty()) {
                    return (DetectedIP[]) out.toArray(new DetectedIP[out.size()]);
                }
            } catch (BindException be) {
                System.err.println(iaddress.toString() + ": " + be.getMessage());
            } catch (UnknownHostException e) {
                System.err.println("Could not find the STUN server " + stunServer + " : " + e
                                   + " - DNS problems? Trying another...");
            } catch (SocketException e) {
                System.err.println("Could not connect to the STUN server: " + stunServer + " : "
                                   + e + " - trying another...");
            } catch (Exception e) {
                System.err.println("Failed to run STUN to server " + stunServer + ": " + e
                                   + " - trying another, report if persistent");
                e.printStackTrace();
            }
        }

        System.err.println("STUN failed: likely detections=" + countLikely
        ...
        synchronized (this) {
            while (true) {
                if (detectors.isEmpty()) {
                    ...
                    DetectedIP[] ips =
                        (DetectedIP[]) detected.toArray(new DetectedIP[detected.size()]);
                ...
                if (nif != null) {
                    try {
                        ...
                        if (m != null) {
                            ...
                            if (iMTU != null) {
                                mtu = iMTU.intValue();
                                System.err.println("Found interface MTU: " + nif.getName() + " : "
                                                   + mtu);
                            }
                        }
                    } catch (Throwable t) {
    ...
    public String handleHTTPGet(HTTPRequest request) throws PluginHTTPException {
        ...
        if (reportedData == null) {
            if (hasRunTestBeenCalled) {
                HTMLNode jSTUNReportInfobox = contentNode.addChild("div", "class",
                                                  "infobox infobox-warning");
                HTMLNode jSTUNReportInfoboxHeader = jSTUNReportInfobox.addChild("div", "class",
                                                        "infobox-header");
                HTMLNode jSTUNReportInfoboxContent = jSTUNReportInfobox.addChild("div", "class",
                                                         "infobox-content");

                jSTUNReportInfoboxHeader.addChild("#", "JSTUN detection report");
                jSTUNReportInfoboxContent.addChild("#",
                        "The plugin hasn't managed to contact any server yet.");
            } else {
                HTMLNode jSTUNReportInfobox = contentNode.addChild("div", "class",
            ...
            HTMLNode jSTUNReportErrorInfobox = contentNode.addChild("div", "class",
                                                   "infobox infobox-normal");
            HTMLNode jSTUNReportInfoboxHeader = jSTUNReportErrorInfobox.addChild("div", "class",
                                                    "infobox-header");
            HTMLNode jSTUNReportInfoboxContent = jSTUNReportErrorInfobox.addChild("div", "class",
                                                     "infobox-content");

            jSTUNReportInfoboxHeader.addChild("#", "JSTUN detection report");
            jSTUNReportInfoboxContent.addChild("#",
                    "The plugin has reported the following data to the node:");
